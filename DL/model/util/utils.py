...
def conv2d_custom(layer, n_filter, filter_size=(3,3), stride=(1,1), act=tf.identity, W_init=w_init,padding='SAME', b_init=b_init, name = 'conv2d'):
    return tl.layers.Conv2d(layer, n_filter=int(n_filter), filter_size=filter_size, strides=stride, act=act, padding=padding, W_init=W_init, b_init=b_init, name=name)
...
class Macron2Stack(Layer):
    def __init__(self, layer=None, name='Macron2StackLayer', angRes=11):
        ...
        self.outputs = tf.space_to_depth(self.inputs, angRes)
        self.all_layers = list(layer.all_layers)
        self.all_params = list(layer.all_params)
        self.all_drop = dict(layer.all_drop)
        self.all_layers.extend([self.outputs])

class Stack2Macron(Layer):
    def __init__(self, layer=None, name='Stack2MacronLayer', angRes=11):

        Layer.__init__(self, name=name)
        self.inputs = layer.outputs
        # def _Macron2Stack(LF, angRes=11):
...
class SAI2ViewStack(Layer):
    def __init__(self, layer=None, name='Macron2StackLayer', angRes=11):
        ...
        # self.outputs = tf.space_to_depth(self.inputs, angRes)
        self.outputs = _SAI2ViewStack(self.inputs, angRes)
        self.all_layers = list(layer.all_layers)
        self.all_params = list(layer.all_params)
        self.all_drop = dict(layer.all_drop)
        self.all_layers.extend([self.outputs])

class Stack2SAI(Layer):
    def __init__(self, layer=None, name='Stack2SAILayer', angRes=11):

        Layer.__init__(self, name=name)
        self.inputs = layer.outputs
        def _Stack2SAI(LF, angRes=11):
            batch, h, w, view_num = LF.get_shape().as_list()
            base_h,base_w=h//angRes,w//angRes
            out = []
            for i in range(angRes):
                out_h = []
                for j in range(angRes):
                    view_idx=i*angRes+j
                    out_h.append(LF[:,:,:,view_idx:view_idx+1])
                out.append(tf.concat(out_h, 2))
            out = tf.concat(out, 1)
            # subpixel upscale
            return out

        self.outputs = _Stack2SAI(self.inputs, angRes)
        self.all_layers = list(layer.all_layers)
        self.all_params = list(layer.all_params)
        self.all_drop = dict(layer.all_drop)
        self.all_layers.extend([self.outputs])
...
class Identity(Layer):
    def __init__(self, layer=None, name='IdentityLayer', **kwargs):

        Layer.__init__(self, name=name)
        self.outputs = tf.identity(layer.outputs)
        self.all_layers = list(layer.all_layers)
        self.all_params = list(layer.all_params)
        self.all_drop = dict(layer.all_drop)
        self.all_layers.extend([self.outputs])

def SAI2Macron(input_layer,angRes,name='SAI2Macron'):
    n=SAI2ViewStack(input_layer,angRes=angRes)
    out= Stack2Macron(n,angRes=angRes)
    return out

class Macron2SAI(Layer):
    def __init__(self, layer=None, name='Macron2SAILayer', angRes=11):
        Layer.__init__(self, name=name)
        ...
        self.outputs = LF2SAI(layer.outputs,angRes=angRes)
        self.all_layers = list(layer.all_layers)
        self.all_params = list(layer.all_params)
        self.all_drop = dict(layer.all_drop)
        self.all_layers.extend([self.outputs])
