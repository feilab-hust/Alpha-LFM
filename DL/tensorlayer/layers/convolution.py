...
class Conv2dLayer(Layer):
    ...
    def __init__(
        ...
        with tf.variable_scope(name):
            W = tf.get_variable(name='W_conv2d_denoise', shape=shape, initializer=W_init, dtype=LayersConfig.tf_dtype, **W_init_args)
            if b_init:
                b = tf.get_variable(name='b_conv2d_denoise', shape=(shape[-1]), initializer=b_init, dtype=LayersConfig.tf_dtype, **b_init_args)
                self.outputs = act(
