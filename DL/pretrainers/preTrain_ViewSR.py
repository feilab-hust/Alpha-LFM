...
class Trainer:
    ...
    def build_graph(self):
        ...
        self.plchdr_SynView = tf.placeholder('float32', [batch_size, *SR_size, 1],name='plh_lfp')

        with tf.device('/gpu:{}'.format(local_configs.TRAIN.device)):
                self.net = sr_model(LFP=self.plchdr_lf, output_size=SR_size, sr_factor=sr_factor, angRes=n_num,
                                              reuse=False, name=SR_tag,transform_layer='SAI2Macron',
                                              channels_interp=ngf2, normalize_mode='percentile')
        self.net.print_params(False)
        g_vars = tl.layers.get_variables_with_name(SR_tag, train_only=True, printable=False)
        ...
        for key in loss_dict:
            ...
            self.losses.update({'ViewSR_' + key: loss_dict[key] * temp_loss})
    ...
    def _save_intermediate_ckpt(self, tag, sess):
        ...
        npz_file_name = checkpoint_dir + '/SR_net_{}.npz'.format(tag)
...
if __name__ == '__main__':
        ...
        lr_init = 5 * 1e-5
        beta1 = 0.9
        n_epoch = local_configs.Pretrain.Training_epoch[1]
        lr_decay = 0.5
        decay_every = 10
        ###=================dir ===========================###
        ...
        finish_flag_file = os.path.join(root_path,'logging','preSR_finish_%s.txt'%local_configs.label)
