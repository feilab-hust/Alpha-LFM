...
class Dataset:
    ...
    def _load_dataset(self, shuffle=True):
        ...
        if self.save_hdf5:
            ...
            self.training_syn2d = np.transpose(np.expand_dims(data_block[self.data_str[0]], 0), (3, 2, 1, 0))  # h w b
            self.training_lf2d = np.transpose(np.expand_dims(data_block[self.data_str[1]], 0), (3, 2, 1, 0))  # h w b
        else:
            self.training_syn2d = _load_imgs(os.path.join(self.base_path,self.data_str[0]),
                                             fn=get_2d_lf,
                                           normalize_fn=self.normalize_fn, type_name=self.data_str[0],
                                           read_type='2_%s' % ('ViewMap'), angRes=self.n_num)
            self.training_lf2d = _load_imgs(os.path.join(self.base_path,self.data_str[1]),
        ...
        ##check
        if (len(self.training_syn2d) == 0) or (len(self.training_lf2d) == 0):
            raise Exception("none of the images have been loaded, please check the file directory in config")
        assert len(self.training_lf2d) == len(self.training_syn2d)

        self.training_pair_num = len(self.training_syn2d)
    ...
    def prepare(self, batch_size, n_epochs):
        ...
        self.epoch = 0
        print('\nHR dataset : %d\nLF dataset: %d\n' % (
        len(self.training_syn2d), len(self.training_lf2d),))
    ...
    def for_test(self):
        n = self.test_img_num
        return np.asarray(self.training_syn2d[0: n]), np.asarray(self.training_lf2d[0: n])
    def hasNext(self):
    ...
    def iter(self):
        ...
        if self.epoch < self.n_epochs:
            ...
            hr_pyramid = [self.training_syn2d[i] for i in shuffle_idx]


            return np.asarray(hr_pyramid), np.asarray(
                [self.training_lf2d[i] for i in shuffle_idx]), idx - nt, self.epoch
